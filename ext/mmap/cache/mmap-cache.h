...
// 64 byte header of metadata file
struct PACKED_STRUCT cache_info_t
{
  ...
  // sum squares used entries per bucket (to determine load)
  uint64_t      entries_squared;
  // origin of expiration times (in seconds since epoch, UTC)
  uint32_t      time_origin;

  unsigned int  __r3: 32;
};


// 8 byte per-page metadata
struct PACKED_STRUCT page_info_t
{
...
// 26 bytes per entry
struct PACKED_STRUCT hash_entry_t
{
...
struct PACKED_STRUCT hash_bucket_t
{
  hash_entry_t entry;
  // padding (all bits set)
  uint16_t     __r1;
...
struct PACKED_STRUCT hash_extent_t
{
